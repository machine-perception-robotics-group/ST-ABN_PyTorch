...
class ClsHead(nn.Module):
    ...
    def __init__(self,
        ...
        if self.with_avg_pool:
            self.avg_pool = nn.AdaptiveAvgPool3d((1, 1, 1)) #nn.AvgPool3d((temporal_feature_size, spatial_feature_size, spatial_feature_size), (1, 1, 1),
                                         #(0, 0, 0))
        if self.fcn_testing:
            self.new_cls = None
            self.in_channels = in_channels
    ...
    def loss(self,
             ...
             labels):
        losses = dict()
        losses['per_loss'] = F.cross_entropy(cls_score, labels)
