...
class TSN3D(BaseRecognizer):
    ...
    def forward_train(self,
        ...
        ax, rx, att, t_att = x
        if self.necks is not None:
            aux_losses = self.necks(ax, gt_label.squeeze())

        # comment out when use cam
        if self.with_spatial_temporal_module:
            rx = self.spatial_temporal_module(rx)
        if self.with_segmental_consensus:
            rx = rx.reshape((-1, num_seg) + rx.shape[1:])
            rx = self.segmental_consensus(rx)
            rx = rx.squeeze(1)
        losses = dict()
        if self.with_cls_head:
            cls_score = self.cls_head(rx)
